...
public class GameMenu {
    // The menu only has three spots: main, settings, and tutorial.
    // Easy to follow, no random confusion stuff, I can change the words on tutorials to what ever I want
    ...
    private void drawTitle(Graphics g, int width) {
        // The title is split in two so it fits the screen better.
        String title = "JIXIAO";
    ...
    private void handleSettingsKeys(KeyEvent e) {
        switch (e.getKeyCode()) {
            ...
            case KeyEvent.VK_ENTER:
                if (selectedIndex == 0) {
                    // This is the one spot where the music switch matters.
                    // Flip it here and it sticks for the next run.
        ...
        switch (screen) {
            ...
            case SETTINGS:
                if (musicToggleBounds.contains(x, y)) {
                    selectedIndex = 0;
                    // Mouse and keyboard should do the same thing.
                    GameSettings.musicEnabled = !GameSettings.musicEnabled;
                    if (!GameSettings.musicEnabled) {
                        Music.stopMusic();
                    }
