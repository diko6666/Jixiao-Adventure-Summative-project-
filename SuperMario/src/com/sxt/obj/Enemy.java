...
public class Enemy implements Runnable {
    ...
    public Enemy(int x, int y, boolean face_to, int type, BackGround bg) {
        ...
        // Type 1 is the walking mushroom enemy.
        thread.start();
    }
    public Enemy(int x, int y, boolean face_to, int type, int max_up, int max_down, BackGround bg) {
        ...
        // Type 2 is the flower that moves up and down.
        thread.start();
    }
    public void death() {
        // Each enemy type reacts a little differently when it gets hit.
        if (this.type == 1) {
    ...
    public void run() {
        while (true) {
            try {
                ...
                // Walk sideways for the ground enemies.
                if (type == 1) {
                    if (face_to) {
                        this.x -= 2;
                    } else {
                    ...
                    // Shells move a bit like the mushrooms, just faster and a little meaner.
                    if (face_to) {
                        this.x -= 2;
                ...
                // If the enemy hits a block or the edge of the map, it turns around.
                ...
                if (type == 2) {
                    // The flower just pops up and down in one spot.
                    if (face_to) {
                        this.y -= 2;
                    } else {
                        this.y += 2;
